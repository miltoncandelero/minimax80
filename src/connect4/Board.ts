		...
		for (let i = 0; i < this.matrix.length /*- this.connect + 1*/; i++) {
			for (let j = 0; j < this.matrix[i].length /*- this.connect + 1*/; j++) {
				...
				for (let c = 1; c < this.connect; c++) {
					...
					if (!this.matrix[i + c] || this.sign(this.matrix[i + c][j]) != current) sameH = false;
					//check the next vertical ones
					if (!this.matrix[i] || this.sign(this.matrix[i][j + c]) != current) sameV = false;
					//check the next diagonals down right
					if (!this.matrix[i + c] || this.sign(this.matrix[i + c][j + c]) != current) sameD = false;
					//check the next diagonals down left
					if (!this.matrix[this.matrix.length - 1 - i - c] || this.sign(this.matrix[this.matrix.length - 1 - i - c][this.matrix[this.matrix.length - 1 - i - c].length - 1 - j + c]) != otherCurrent) sameOtherD = false;
				}
